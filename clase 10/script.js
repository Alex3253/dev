...
if(modifyForm){
    modifyForm.addEventListener('submit', function(event){
        ...
        const quantity = parseInt(document.getElementById('modify-description').value);
...
function displaySalesList(conatiner){
    conatiner.innerHTML = '';
    sales.forEach((sale, index) => {
        ...
        `;
        conatiner.appendChild(saleDiv);
    });
}

function displaySales(conatiner){
    conatiner.innerHTML = '';
    sales.forEach((sale, index) => {
        const saleDiv = document.createElement('div');
        saleDiv.className = 'grid-item';
        saleDiv.innerHTML = `
            ...
            <p>Total: $${sale.total.toFixed(2)}</p>
            <p>Estado: ${sale.status}</p>
            <button onclick="confirmSale(${index})">Confirmar Ventas</button>
        `;
        conatiner.appendChild(saleDiv);
    });
}
